...
<body>
                            ...
                            <tbody>
                                <?php   if (!empty($_SESSION['product'])){ 
                                    ...
                                    foreach ( $productcart as $singleproduct) { ?>
                                     <tr> <td class="shoping__cart__item" style="width: 400px">
                                        <?php echo"<img  src='admin/uploads/{$singleproduct['pro_image']}' width='150' hight='150' alt=''>"; ?>
                                        <h5><?php echo $singleproduct['pro_name']; ?></h5>
                                    </td>
                                    <td class="shoping__cart__price">
                                        <?php echo $singleproduct['pro_price']; ?> JD
                                    </td>
                                    <td class="shoping__cart__quantity"><center>
                                      
                                      <form oninput="x.value=parseFloat(a<?php echo "{$singleproduct['pro_id']}"; ?>.value)*parseFloat(b<?php echo "{$singleproduct['pro_id']}"; ?>.value)">
                                        
                                        <input type="hidden" id="pid"  value="<?php echo "{$singleproduct['pro_id']}"; ?>">

                                        <input type="hidden" id="a<?php echo "{$singleproduct['pro_id']}"; ?>" value="<?php echo "{$singleproduct['pro_price']}"; ?>">

                                        <input type="number" min="1" id="b<?php echo "{$singleproduct['pro_id']}"; ?>" class="b" data-id="<?php echo "{$singleproduct['pro_id']}"; ?>" value="<?php echo $_SESSION['order'][$i]['qty']; ?>">

                                    </center> </td>
                                    <td class="shoping__cart__total">
                                        <output  name="x" for="a<?php echo "{$singleproduct['pro_id']}"; ?> b<?php echo "{$singleproduct['pro_id']}"; ?>"><?php echo "{$_SESSION['order'][$i]['price']}"; ?></output>
                                    </form>
                                </td>
                                <td class="shoping__cart__item__close">
                                    <span> <a title="Remove Item"  class="btn btn-danger btn-sm" id="btnsearch" name="btnsearch" href="controller.php?action=delete&id=<?php echo $singleproduct['pro_id']; ?>"> <i class="fa fa-trash-o"></i>
                                    </a> </span> </td>
                                </tr>
                                <?php $i+=1;}}?>
                            </tbody>
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <div class="shoping__checkout">
                        <h5>Cart Total</h5>
                        <ul>
                            <li>Total <span id=sum><?php echo $sum ; ?> JD</span></li>
                        </ul>
 ...
 $(".b").bind('keyup mouseup', function () {
    ...
    output.innerHTML =  sum.toFixed(2);
     var qty = document.getElementById('b'+pid).value;
    var price = document.getElementById('a'+pid).value;



              $.ajax({   
                    type:"GET",
                    url: "controller.php?action=edit&qty="+qty +"&price="+price ,              
                    data:{id:pid , qty: qty},               
